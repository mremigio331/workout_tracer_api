...
def lambda_handler(event, context):
    ...
    try:
        logger.info(
            f"Fetching Strava activities for user_id: {user_id}, next_token: {next_token}"
        )
        activities, next_token, total_pages = (
            strava_client.get_athlete_activities_with_token(
        ...
        logger.info(
            f"Successfully fetched {len(activities)} Strava activities for user_id: {user_id} (next_token: {next_token}, total_pages: {total_pages})"
        )
    ...
    # If there are more pages, invoke new Lambda for next page
    if next_token:
        lambda_name = os.getenv("AWS_LAMBDA_FUNCTION_NAME")
        payload = {"user_id": user_id, "next_token": next_token}
        try:
            logger.info(
                f"Invoking next onboarding lambda for user_id {user_id} with next_token {next_token}"
            )
            ...
            logger.error(f"Failed to invoke next onboarding lambda: {e}")
