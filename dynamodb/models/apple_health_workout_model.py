...
class AppleHealthWorkoutModel(BaseModel):
    # Core identifiers & properties (from HKWorkout)
    workout_uuid: str
    ...
    # Statistics (from HKWorkout.statistics(for:) / deprecated direct properties)
    total_energy_burned: Optional[float] = None
    total_distance: Optional[float] = None
    total_swimming_stroke_count: Optional[int] = None
    total_flights_climbed: Optional[int] = None
    step_count: Optional[int] = None

    # Metadata (from HKWorkout.metadata with HKMetadataKey* constants)
    ...
    def to_strava_format(self) -> dict:
        ...
        return {
            ...
            "kilojoules": (
                (self.total_energy_burned / 0.239006)
                if self.total_energy_burned
                else None
            ),
