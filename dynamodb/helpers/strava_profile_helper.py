...
class StravaProfileHelper:
    ...
    def create_strava_profile(
        ...
        profile = StravaAthleteModel(
            ...
            resource_state=int(locals().get("resource_state")) if locals().get("resource_state") is not None else None,
            ...
            badge_type_id=int(locals().get("badge_type_id")) if locals().get("badge_type_id") is not None else None,
            # Fix: Only cast to int if the value is a whole number, otherwise set to None or handle gracefully
            weight=int(float(locals().get("weight"))) if locals().get("weight") is not None and float(locals().get("weight")).is_integer() else None,
        ...
        try:
            ...
            try:
                ...
                self.logger.info(f"Parsed before_item into StravaAthleteModel: {before}")
            except Exception as parse_exc:
                self.logger.error(f"Error parsing before_item into StravaAthleteModel: {parse_exc}")
                before = None
            ...
            self.logger.info(f"Successfully created audit record for user_id: {user_id}")
            return profile
        except ClientError as e:
            self.logger.error(f"Error creating/updating Strava profile for {user_id}: {e}")
            raise
        except Exception as e:
            self.logger.error(f"Unexpected error in create_strava_profile for user_id: {user_id}: {e}")
            raise
    ...
    def get_strava_profile(self, user_id: str) -> dict | None:
        ...
        try:
            ...
            self.logger.error(f"Error retrieving Strava profile: {e.response['Error']['Message']}")
            return None
        except Exception as e:
            self.logger.error(f"Unexpected error in get_strava_profile for user_id: {user_id}: {e}")
            return None
    ...
    def update_strava_profile(
        ...
        try:
            ...
            self.logger.info(f"Updated Strava profile for user_id: {user_id}: {updated_item}")
            if updated_item:
                ...
                self.logger.warning(f"No attributes returned after update for user_id: {user_id}")
                return None
        except ClientError as e:
            self.logger.error(f"Error updating Strava profile: {e.response['Error']['Message']}")
            return None
        except Exception as e:
            self.logger.error(f"Unexpected error in update_strava_profile for user_id: {user_id}: {e}")
            return None
    ...
    def convert_floats_to_decimal(obj):
        ...
        if isinstance(obj, dict):
            return {
                k: StravaProfileHelper.convert_floats_to_decimal(v) for k, v in obj.items()
            }
    ...
    def _decimals_to_floats(obj):
        ...
        """
        if isinstance(obj, dict):
            return {k: StravaProfileHelper._decimals_to_floats(v) for k, v in obj.items()}
        elif isinstance(obj, list):
    ...
    def serialize_model(model: Any) -> dict:
        ...
        if isinstance(model, dict):
            return {
                k: StravaProfileHelper.serialize_model(v) for k, v in model.items()
            }
